...
export default function Users({ users, filters }) {
  ...
  // 💡 RÉGLAGE DU PROBLÈME : Référence pour ignorer le rendu initial lors du changement de page
  ...
  // 🔍 Recherche dynamique (debounce) - CORRIGÉ
  useEffect(() => {
    // Si c'est le premier rendu (chargement de la page ou clic sur pagination), on ne fait rien
    ...
    const delay = setTimeout(() => {
      router.get(
        ...
        { search },
        {
          preserveState: true,
          replace: true,
          preserveScroll: true, // Évite les sauts de page au chargement
        }
      );
    }, 500);
      ...
      <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        {showSuccess && flash?.success && (
          <div className="mb-6 p-4 bg-green-50 border-l-4 border-green-500 text-green-700 rounded-lg shadow-sm flex justify-between items-center animate-slide-down">
            ...
            <button onClick={() => setShowSuccess(false)} className="text-green-500 hover:text-green-700 transition-colors">
        ...
        <div className="mb-8">
          <div className="flex justify-between items-center mb-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Gérez les comptes utilisateurs et leurs rôles</p>
            </div>
            <Link href="/admin/users/create" className="inline-flex items-center px-4 py-2 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg hover:from-blue-700 hover:to-blue-800 transition-all duration-200 shadow-sm hover:shadow-md gap-2 font-medium">
        ...
        <div className="mb-6 flex flex-col sm:flex-row gap-4">
          ...
          {search && (
            <button onClick={() => setSearch("")} className="inline-flex items-center px-4 py-2.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-all duration-200 gap-2 font-medium">
              <XMarkIcon className="h-5 w-5" />
              Réinitialiser
            </button>
          )}
                ...
                {users.data.map((user) => (
                  <tr key={user.id} className="hover:bg-gray-50 transition-colors duration-150">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right">
                      <Link href={`/admin/users/${user.id}/edit`} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-blue-600 hover:text-blue-700 hover:bg-blue-50 rounded-lg transition-all duration-200 font-medium text-sm">
                        <PencilSquareIcon className="h-4 w-4" /> Modifier
                      </Link>
                      <button onClick={() => confirmDelete(user)} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg transition-all duration-200 font-medium text-sm">
            ...
            <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
              <div className="flex flex-col sm:flex-row gap-4 justify-between items-center">
                <div className="text-sm text-gray-600">
                  Affichage de <span className="font-medium">{users.from || 0}</span> à <span className="font-medium">{users.to || 0}</span> sur <span className="font-medium">{users.total || 0}</span> utilisateurs
                </div>
                <div className="flex gap-2">
                  {users.links.map((link, index) =>
                    link.url ? (
                      <Link key={index} href={link.url} className={`px-3 py-1.5 text-sm rounded-lg transition-all duration-200 ${link.active ? "bg-blue-600 text-white shadow-sm" : "bg-white text-gray-700 hover:bg-gray-100 border border-gray-300"}`} dangerouslySetInnerHTML={{ __html: link.label }} />
                    ) : (
                      <span key={index} className="px-3 py-1.5 text-sm text-gray-400 bg-gray-100 rounded-lg cursor-not-allowed" dangerouslySetInnerHTML={{ __html: link.label }} />
                    )
                  )}
                </div>
              </div>
            ...
            <div className="p-6">
              ...
              <div className="bg-red-50 rounded-lg p-4 mb-6 border border-red-100">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    <p className="text-gray-900 font-bold">{deletingUser?.name}</p>
                    <p className="text-sm text-gray-600">{deletingUser?.email}</p>
                  </div>
                </div>
              </div>
              <div className="flex gap-3 justify-end">
                <button onClick={() => setShowDeleteModal(false)} className="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-all duration-200 font-medium">Annuler</button>
                <button onClick={deleteUser} className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-all duration-200 font-medium shadow-sm">Supprimer définitivement</button>
              </div>
            </div>
          </div>
        </div>
