...
export default function TrafficDashboard({ auth, data, filters, stats }) {
    // États locaux pour les inputs de date
    const [startDate, setStartDate] = useState(filters.start_date);
    const [endDate, setEndDate] = useState(filters.end_date);

    // Synchronisation si les filtres changent depuis le serveur
    useEffect(() => {
    ...
    // --- LOGIQUE DE FILTRAGE ---
    const handleFilter = () => {
        router.get('/analyste-op/traffic', 
            { start_date: startDate, end_date: endDate }, 
            { preserveState: true }
        );
    };

    // --- CALCULS POUR LES LIGNES DE RÉFÉRENCE ---
                    ...
                    <div className="flex items-center gap-3">
                        <div className="flex items-center bg-slate-50 rounded-xl px-3 border border-slate-200">
                            <input 
                                ...
                                className="bg-transparent border-none text-sm focus:ring-0 py-2" 
                            />
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={data} margin={{ left: 20, bottom: 40, right: 20 }}>
                                ...
                                <XAxis 
                                    ...
                                    height={60} 
                                    tick={{fontSize: 10}} 
                                    label={{ value: 'Date de création', position: 'insideBottom', offset: -25, fontSize: 12, fill: '#64748b' }} 
                                />
                                <YAxis 
                                    tickFormatter={formatYAxis} 
                                    tick={{fontSize: 11}} 
                                    label={{ value: 'Nombre de CDR', angle: -90, position: 'insideLeft', fontSize: 12, fill: '#64748b' }} 
                                />
                ...
                <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-10 gap-4">
                        {data && data.length > 0 ? (
                            ...
                            <p className="col-span-full text-center text-slate-400 text-sm py-4">Aucune donnée sur cette période</p>
                        )}
                    </div>
                </div>
            </div>
